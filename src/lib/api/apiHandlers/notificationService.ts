...
export const sendPushNotification = async (
  ...
  try {
    ...
    const functionUrl = `https://us-central1-${config.firebaseProjectId}.cloudfunctions.net/send_notification`;
    console.log(
      "Sending notification to Cloud Function:",
      functionUrl,
      "with payload:",
      notification,
    );
    ...
    const response = await fetch(functionUrl, {
      ...
      body: JSON.stringify({
        notification: {
          ...notification,
          usertype: notification.userType || notification.usertype,
          devicetype: notification.deviceType || notification.devicetype,
          msg: notification.body || notification.msg,
        },
      }),
    });
